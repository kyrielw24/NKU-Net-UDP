...
int main()
{
    ...
    cin >> ssthresh;
    int* S_W = new int[2];
    Client_Server_Size(server, addr, length, SIZE, ssthresh, S_W);
    SIZE = S_W[0];
    ssthresh = S_W[1];
    ...
    if (label == 0) {
        while (1) {
            ...
            int name_len = Recv_Message(server, addr, length, F_name, SIZE, ssthresh);

            // 全局结束
            if (name_len == 999)
                break;

            int file_len = Recv_Message(server, addr, length, Message, SIZE, ssthresh);
            string a;
    ...
    if (label == 1) {
        while (1) {
            ...
            Send_Message(server, addr, length, InFileName, strlen(InFileName), SIZE, ssthresh);
            clock_t start = clock();
            Send_Message(server, addr, length, FileBuffer, F_length, SIZE, ssthresh);
            clock_t end = clock();
